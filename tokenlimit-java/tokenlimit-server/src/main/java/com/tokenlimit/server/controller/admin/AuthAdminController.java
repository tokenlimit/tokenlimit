 ...
 * 会话基于 Redis（{@link AuthSession}）：支持多实例共享、滑动过期、并发会话数限制、
 * 服务端实时踢人；登录失败连续超限触发临时锁定（Redis 计数，多实例共享）。</p>
 */
@RestController
...
public class AuthAdminController {
    ...
    public AuthAdminController(UserMapper userMapper, AuditLogMapper auditLogMapper, AuthSession authSession) {
    ...
    public Result<Map<String, Object>> login(@Valid @RequestBody LoginRequest req) {
        ...
        if (!passwordOk || user.getLoginEnabled() == null || !user.getLoginEnabled()) {
            authSession.recordLoginFailure(req.getUsername());
        ...
        authSession.resetLoginFailures(req.getUsername());
        ...
        // 创建 Redis 会话（完整身份快照，避免后续请求查库）
        AuthSession.SessionInfo session = new AuthSession.SessionInfo(user, mustChangePassword);
        session.setRole(resolveRole(user));
        String token = authSession.create(session);
    ...
    public Result<Void> changePassword(@Valid @RequestBody ChangePasswordRequest req) {
        String token = SecurityUtils.currentToken();
        AuthSession.SessionInfo session = SecurityUtils.requireSession();
        User user = userMapper.selectOne(new LambdaQueryWrapper<User>()
        ...
        // 改密后：清除当前会话强制改密标记，并踢掉其它设备（保留当前登录）
        authSession.markPasswordChanged(token);
        authSession.destroyOthers(token);
    ...
    public Result<Void> logout() {
        authSession.destroy(SecurityUtils.currentToken());
        return Result.success();
    }
